...
import { 
  ...
  RefreshCw, ArrowRight, ShieldCheck, MapPin, Zap, AlertCircle
} from 'lucide-react';
...
const SdlcStudentDashboard = ({ onStartTest, onViewResult }) => {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-28">
        <ClockLoader size="md" color="#e45d13" text="Loading assessments..." />
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-6 text-left font-sans">
      ...
      {/* ========================================================================= */}
      {/* 1. CLEAN STUDENT PROFILE HEADER                                           */}
      {/* ========================================================================= */}
      <div className="bg-white border border-slate-200/90 rounded-2xl p-6 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-5">
        <div className="space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-[11px] font-bold text-amber-700 bg-amber-50 border border-amber-200 px-2.5 py-0.5 rounded-md uppercase tracking-wider">
              SDLC Candidate Portal
            </span>
            {user?.center && (
              <span className="text-[11px] font-medium text-slate-600 bg-slate-100 px-2.5 py-0.5 rounded-md flex items-center gap-1">
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-500 font-medium">
            <div>
              Enrollment ID: <strong className="text-slate-800 font-mono">{user?.enrollmentId || 'N/A'}</strong>
            </div>
            {user?.courseTrack && (
              <div>
                Course Track: <strong className="text-slate-800">{user.courseTrack}</strong>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="flex items-center gap-2 border-b border-slate-200">
        <button
          ...
          className={`pb-3 px-1 text-sm font-bold border-b-2 transition-colors cursor-pointer flex items-center gap-2 ${
            activeTab === 'available'
              ? 'border-[#e45d13] text-[#e45d13]'
              : 'border-transparent text-slate-500 hover:text-slate-800'
          }`}
        >
          <span>Available Assessments</span>
          <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${
            activeTab === 'available' ? 'bg-amber-100 text-amber-900' : 'bg-slate-100 text-slate-600'
          }`}>
            {activeTests.length}
          </span>
        </button>
        ...
        >
          ...
          <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${
            activeTab === 'results' ? 'bg-amber-100 text-amber-900' : 'bg-slate-100 text-slate-600'
          }`}>
        ...
        <div>
          {activeTests.length === 0 ? (
            <div className="bg-white border border-slate-200/80 rounded-2xl p-12 text-center space-y-3">
              ...
              <h3 className="text-base font-bold text-slate-800">No Tests Available</h3>
              <p className="text-xs text-slate-500 max-w-sm mx-auto">
                There are currently no active tests scheduled for your track. Check back later or click refresh.
              </p>
            </div>
          ) : (
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-[11px] font-bold text-slate-700 bg-slate-100 px-2 py-0.5 rounded">
                        {test.subject || 'Technical Assessment'}
                      </span>
        ...
        <div className="bg-white border border-slate-200/90 rounded-2xl shadow-2xs overflow-hidden">
          {completedTests.length === 0 ? (
            <div className="p-12 text-center space-y-2">
              ...
              <h3 className="text-sm font-bold text-slate-700">No Past Results</h3>
              <p className="text-xs text-slate-400">You haven't completed any assessments yet.</p>
            </div>
          ) : (
                  ...
                  {completedTests.map((t) => {
                    ...
                    return (
                      <tr key={t._id} className="hover:bg-slate-50/50 transition">
                        ...
                        <td className="py-4 px-5 font-mono font-bold text-slate-800">
                          {isResultsReleased ? `${t.userPercentage ?? t.percentage ?? 0}%` : 'Held by Admin'}
                        </td>
                        <td className="py-4 px-5">
                          {isResultsReleased ? (
