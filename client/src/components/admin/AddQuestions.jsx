...
const AddQuestions = ({ test, onFinished }) => {
  ...
  const handleImportSubmit = async () => {
    ...
    const finalPayload = validQuestions.map((q, idx) => {
      ...
      const selected = filteredOptions.find(o => o.label === correct);
      if (!selected || !selected.text.trim()) {
        correct = filteredOptions[0]?.label || 'A';
      }
    ...
    try {
      ...
      toast.success(`Successfully imported ${finalPayload.length} questions.`, { id: loader });
      setNotepadText('');
      setParsedQuestions([]);
