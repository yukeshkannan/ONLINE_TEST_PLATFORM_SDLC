...
import { FileText, Users, Award, Download, Clock, Trash2, AlertTriangle, X, ChevronDown, Check, GraduationCap, BookOpen, FileSpreadsheet } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const { stats, recentActivity } = data || {
    stats: { totalTests: 0, totalStudents: 0, todaysAttempts: 0, totalSubmissions: 0, overallPassRate: 0 },
    recentActivity: []
  };
    ...
    <div className="space-y-6 animate-fadeIn font-sans text-left pb-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 font-medium mt-0.5">
            Monitor real-time candidate registrations, exam submissions, and export system analytics.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          ...
          <div className="mt-3">
            <span className="text-3xl font-extrabold text-slate-900 font-poppins">{stats.totalTests}</span>
            <p className="text-[11px] text-slate-400 font-medium mt-0.5">Active and draft test papers</p>
          </div>
        </div>
        ...
        >
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Registered Candidates</span>
            <Users className="h-4 w-4 text-slate-400" />
          </div>
          <div className="mt-3">
            <span className="text-3xl font-extrabold text-slate-900 font-poppins">{stats.totalStudents}</span>
            <p className="text-[11px] text-slate-400 font-medium mt-0.5">Enrolled student accounts</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-2xs flex flex-col justify-between">
          ...
          <div className="mt-3">
            <span className="text-3xl font-extrabold text-slate-900 font-poppins">{stats.todaysAttempts}</span>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-2xs flex flex-col justify-between">
          ...
          <div className="mt-3">
            <span className="text-3xl font-extrabold text-slate-900 font-poppins">{stats.overallPassRate}%</span>
            <p className="text-[11px] text-slate-400 font-medium mt-0.5">System success percentage</p>
          </div>
        </div>
      ...
      <div className="bg-white border border-slate-200 shadow-2xs rounded-xl p-5 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 font-medium">Live feed of test submissions turned in by candidates.</p>
          </div>
              ...
              {filteredRecentActivity.length === 0 ? (
                <tr>
                  <td colSpan="6" className="py-12 text-center text-slate-400 font-semibold text-xs">
                    No recent test submissions logged.
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={attempt._id} className="hover:bg-slate-50/60 transition-colors">
                    <td className="py-3 px-4 text-left">
                      <div className="space-y-0.5">
                        ...
                        <p className="text-[10px] font-mono text-slate-400">{attempt.studentId?.rollNumber || attempt.studentId?.enrollmentId || 'N/A'}</p>
                      </div>
                    </td>

                    <td className="py-3 px-4 text-left">
                      <span className="bg-slate-100 text-slate-700 px-2 py-0.5 rounded text-[11px] font-semibold">
                        {attempt.studentId?.department || attempt.studentId?.courseTrack || 'General'}
                      </span>
                    </td>
