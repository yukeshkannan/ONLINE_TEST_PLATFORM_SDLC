...
import { Building2, Plus, Search, Edit3, Trash2, RefreshCw, AlertTriangle, CheckCircle2 } from 'lucide-react';
...
const CohortManagement = () => {
  ...
  const [deleteTarget, setDeleteTarget] = useState(null); // { id, code, name }
  ...
  const fetchDepartments = async () => {
    ...
    try {
      ...
      console.error(err);
      toast.error('Unable to load department registry.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    const loader = toast.loading(`Deleting department '${code}'...`);
    try {
      await api.delete(`/cohorts/departments/${id}`);
      setDepartments(prev => prev.filter(d => d._id !== id));
      toast.success(`Department '${code}' deleted successfully.`, { id: loader });
    } catch (err) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-500 font-medium">
            Manage academic departments and short codes. Changes automatically update student registration and assessment targeting dropdowns.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex items-center justify-between">
          <div>
            ...
            <h3 className="text-2xl font-black text-emerald-600 font-mono mt-0.5">{activeCount}</h3>
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm flex items-center">
          <div className="relative w-full">
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 hover:border-slate-300 focus:border-[#004f90] rounded-xl py-2.5 pl-9 pr-4 text-xs font-semibold text-slate-800 placeholder:text-slate-400 focus:outline-none transition-all"
            />
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="py-16 text-center text-xs text-slate-400 space-y-2">
            ...
            <p>Click "Add Department" above to add a new academic department.</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredDepartments.map((dept) => (
                  <tr key={dept._id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    {/* Name */}
                    <td className="py-4 px-5 font-bold text-slate-800 text-sm">{dept.name}</td>
              ...
              <form onSubmit={handleSaveDepartment} className="space-y-4 text-left">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-200 focus:border-[#004f90] rounded-xl py-2.5 px-3.5 text-xs font-bold text-slate-800 focus:outline-none uppercase"
                  />
                </div>
          ...
          <>
            ...
            >
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-500 font-medium">
                  Are you sure you want to delete <span className="font-extrabold text-slate-800">'{deleteTarget.code}'</span>?
                </p>
              </div>
