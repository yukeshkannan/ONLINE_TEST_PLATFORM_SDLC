export const sendCredentialsEmail = async (email, name, rollNumber) => {
  ...
  const senderEmail = (process.env.SENDER_EMAIL || 'yukesh785.in@gmail.com').trim();
  ...
  if (!apiKey) {
    console.error('SERVER_ERROR: BREVO_API_KEY environment variable is missing.');
    throw new Error('BREVO_API_KEY is not configured in server environment variables.');
  }
  ...
  try {
          ...
          <body style="margin: 0; padding: 0; background-color: #f1f5f9; -webkit-text-size-adjust: 100%; -ms-text-size-adjust: 100%;">
            <div class="outer-wrapper" style="background-color: #f1f5f9; padding: 48px 16px; min-height: 100%;">
              <table cellpadding="0" cellspacing="0" border="0" class="main-card" style="max-width: 580px; margin: 0 auto; background-color: #ffffff; border-radius: 16px; overflow: hidden; border: 1px solid #e2e8f0; box-shadow: 0 4px 20px rgba(0, 79, 144, 0.03); width: 100%; border-collapse: collapse;">
                <!-- Header Section -->
                <tr>
                  <td class="header-cell" style="background-color: #ffffff; padding: 28px 40px; text-align: center; border-bottom: 1px solid #eef2f6;">
    ...
    const responseData = await response.json().catch(() => ({ message: `HTTP ${response.status} ${response.statusText}` }));

    if (!response.ok) {
      console.error('Brevo API Error Response:', response.status, responseData);
      const errMsg = responseData.message || responseData.code || responseData.error || `Brevo HTTP ${response.status}`;
      throw new Error(`Brevo Error: ${errMsg}`);
    }

    return responseData;
  } catch (err) {
    console.error('sendCredentialsEmail Error:', err.message);
    throw err;
  }
};
...
export const sendOTPEmail = async (email, name, otp) => {
  ...
  try {
    ...
    if (!response.ok) {
      console.error('Brevo OTP Error Response:', response.status, responseData);
      const errMsg = responseData.message || responseData.code || responseData.error || `Brevo HTTP ${response.status}`;
      throw new Error(`Brevo Error: ${errMsg}`);
    }
